    ...
    for i in range(R):
        for j in range(C):
            ...
            for d in range(4):
                ...
                if not within(i_to, j_to):
                    continue
                if (i_to, j_to) not in blizzards_no_d[t1 + t]:
                    new_reachable[i_to][j_to] = True
            if (i, j) not in blizzards_no_d[t1 + t]:
                new_reachable[i][j] = True
    reachable = new_reachable
    if reachable[0][1]:
        t2 = t
    ...
    for i in range(R):
        for j in range(C):
            ...
            for d in range(4):
                ...
                if not within(i_to, j_to):
                    continue
                if (i_to, j_to) not in blizzards_no_d[t1 + t2 + t]:
                    new_reachable[i_to][j_to] = True
            if (i, j) not in blizzards_no_d[t1 + t2 + t]:
                new_reachable[i][j] = True
    reachable = new_reachable
    if reachable[R - 1][C - 2]:
        t3 = t
